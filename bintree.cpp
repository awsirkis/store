...
//             by checking each Node and its Merchandise.
//---------------------------------------------------------------------------
bool BinTree::findEquality(Node* root, Node* otherRoot) const
{
  if ((root != nullptr && otherRoot != nullptr) && (*root->data == *otherRoot->data))
  { //Base case 1: If the node is not empty and the Merchandise is the same, they are the same
    return true;
  }
  if ((root == nullptr) || (otherRoot == nullptr))
  { //Base case 2: If either of them are empty, they are different
    return false;
  } //Return true if the last leaf's Merchandise matches after recursive calls, false if not
